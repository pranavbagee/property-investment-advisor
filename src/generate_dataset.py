...
def save_all(base_dir="."):
    """Callable entry point (used by app.py's auto-bootstrap on first run)."""
    os.makedirs(os.path.join(base_dir, "data"), exist_ok=True)
    props = generate_properties()
    appr = generate_city_appreciation()
    props.to_csv(os.path.join(base_dir, "data", "properties.csv"), index=False)
    appr.to_csv(os.path.join(base_dir, "data", "city_appreciation.csv"), index=False)
    return props, appr
